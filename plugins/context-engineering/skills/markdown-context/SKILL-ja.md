...
- ファイルが大きい / 長さが不明 / 一部だけ要る → 以下の mdidx 手順。
- 「コードブロックだけ」「特定要素型の横断抽出」「変換」が要る → mq(後述)。

## mdidx(主役・固定2手)

`mdidx` は Markdown を `{index, sections}` の JSON に変換する。
...
2. 必要な節だけ取る(N は索引の番号):
   ...
   複数節なら `jq -r '.sections[3,5,8]'`。見出しを子節ごとまとめて取るなら範囲スライス
   `jq -r '.sections[2:6][]'`。番号は親の直後に子が続くので、連続した深い見出しの範囲が
   その見出しのサブツリー全体になる。`sections[0]` は最初の見出しより前の前文。
...
**節の取得が目的なら mdidx を使う。** mq は次のような別の用途のときだけ:
...
- mdidx で済む「節取得」を mq のクエリで書かない(誤クエリで取りこぼす)。
