...
compatibility: 文章規範を japanese-tech-writing へ委ねているため、同じ skills ディレクトリに japanese-tech-writing が配置されている必要がある。
---
...
この skill は `japanese-tech-writing` を必須の依存として使う。
論証の順序、不確実性の保持、証拠と主張範囲といった規範はそちらが正本であり、ここには複製しない。

作業前に `../japanese-tech-writing/SKILL.md` を読む。
特に「段落と論証の構成」「論証の厳密さ」「冗長の排除」に従う。

参照先が読めない場合は、以下の検出手順だけで代用せず、編集を始める前に止まる。
`japanese-tech-writing` を同じ skills ディレクトリへ導入するよう利用者に伝える。
