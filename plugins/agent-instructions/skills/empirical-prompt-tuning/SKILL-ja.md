...
1. **ベースライン準備**: 対象プロンプトを確定し、次の 2 つを用意する。
   ...
   - 対象がこのリポジトリの skill なら、シナリオを会話中で作らない。その skill の `evals/cases.json` から読み込むか、無ければ先にそこへ書く。後述の「永続 eval corpus」を参照。
...
会話の中にしかないシナリオとチェックリストは再実行できない。
このリポジトリの skill が対象なら、ディスク上に置く。
...
加えて `unevaluated` という枠外の状態を1つ持つ。第四の判定ではなく「測定が存在しない」ことを表し、証拠を捕れなかった項目に使う。
`×` と区別するのは、保存された run が「判断できなかった」を失敗として記録しないためである。accuracy の分母から外れ、`[critical]` が未判定なら success は false ではなく不明になる。
ただし別の `[critical]` が既に落ちているなら、観測できた失敗のほうが優先する。
...
採点側には runner が観測した tool call と file 変更も渡す。「実際に読んだか」「実際に書いたか」を問う要件は出力からは判定できず、やっていない実行者とやった実行者の区別がつかないためである。
