...
手順3・手順5・手順6は、コードと実装者の主張が**正しいか**を問う。よって priors が自分と独立した別モデルファミリの相談先(Claude Code なら Codex)を、コスト増に見合うものとして優先する。
...
```
Args: よく相談して。別モデルファミリの相談先を優先してほしい。PR #{番号} の概要欄の説明と実際の差分に齟齬がないか確認してほしい。{概要欄の要約と確認ポイント}
```
...
```
Args: よく相談して。同ファミリのsubagentでよい。PR #{番号} で追加・変更された命名と設計パターンが、既存のコードベースの慣習と一致しているか確認してほしい。{変更概要}
```
...
```
Args: よく相談して。別モデルファミリの相談先を優先してほしい。PR #{番号} のコードレビューをお願いします。{変更概要と確認ポイント}
```
...
疑わしい点がある場合は、「外部Agent相談の共通方針」に従い外部Agentにも相談する。Argsに「よく相談して」と、別モデルファミリの相談先を優先する旨を含めること。
...
- **subagent-consultation**: Agentツール(subagent)と相談するスキル。手順3・手順4・手順5・手順6で外部Agentとして使用する。Codex を含む相談先の選定もこのskillが行う
