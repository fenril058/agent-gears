...
コマンドやファイルの具体例としてNode.jsを使って説明していますが、実際に変更されたライブラリやプログラム言語の常識に照らし合わせて適宜読み替えてください。
...
このpull requestで何が変更されたか、その範囲を把握してください。

lock file差分から、更新・追加・削除されたパッケージを列挙してください。

- 件数が多い場合でも、主要な変更対象は漏れなく明示する
- 更新対象ライブラリとの関係が明確ではないパッケージの変更が含まれていたら、その旨を注記する
- この時点で安全性や影響範囲は断定しない。後続セクションで確認すべき注意点として扱う

例: npmの `package-lock.json` の場合、`gh pr diff` や `git diff` でlock file差分を取得する。個別パッケージの周辺コンテキストは `grep -C15 '<パッケージ名>' package-lock.json` で確認できる。
...
調査結果は、参照箇所をファイル単位で一覧にまとめてください。このリストはセクション7-2「バージョン整合性の確認」の入力として使います。
セクション3・4等の他セクションで読んだファイルにもバージョン指定が含まれている場合があるので、漏らさずリストに含めてください。
...
3. PRで更新されたバージョンと揃えて、同時に更新するべき箇所が他にないかリポジトリ全体を調査する
   - 「5. ライブラリの使用箇所」で作成した参照箇所のリストも確認する
...
- あるセクションで読んだ・触れたファイルが、セクション5の参照箇所リストに含まれているか
- セクション5で列挙した参照箇所が、セクション7-2の整合性確認で全て検証されているか
- セクション4で確認した依存関係の情報と、セクション6の変更内容の要約に矛盾がないか
- セクション3で列挙した変更パッケージのうち、セクション6のrelease note要約で根拠や意図が説明されていないものがないか。説明されていない変更があれば、未説明の変更としてレビュー上の注意点に含まれているか確認する
