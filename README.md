...
ブランチ単位で設計判断・却下理由・制約を引き継ぐ)、wiki-knowledge-export(**永続層**:
ブランチを越えて残す知見=実測値・規約・システム横断の落とし穴を wiki へ)、
library-update-review(依存更新 PR のレビュー)を含む。揮発/永続の判定で発見を振り分ける。
...
plugins/
  ...
  agent-collaboration/            plugin: エージェント協調(一部 shokai/agent-skills 由来・MIT)
    ...
    skills/
      ...
      wiki-knowledge-export/      永続層: ブランチを越える知見を wiki へ(自作、+ TEMPLATE.md)
...
- **`TEMPLATE.md` は日本語のまま**: `agent-collaboration` の `sanity-review` /
  `conversation-context-export` / `wiki-knowledge-export`。これは GitHub に貼る/wiki に
  残す成果物の雛形(出力)であり、指示本体(`SKILL.md`)のみ英語化する。出力は利用者の
  作業言語に従う。
...
- **agent-collaboration**(`subagent-consultation` / `sanity-review` /
  ...
  2 段フォールバックに書き換えてある。
  なお `agent-collaboration` の `wiki-knowledge-export` は**自作**(shokai 由来ではない)。
  揮発層(`conversation-context-export`)の対として、ブランチを越える永続知見を wiki へ
  書き出す。
