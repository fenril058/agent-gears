...
結果ではなくプロセスをレビューする)、conversation-context-export/import(セッション間で
設計判断・却下理由・制約を引き継ぐ)、library-update-review(依存更新 PR のレビュー)を含む。
...
plugins/
  ...
  agent-collaboration/            plugin: エージェント協調(shokai/agent-skills 由来・MIT)
    .claude-plugin/plugin.json
    skills/
      subagent-consultation/      サブエージェントへのセカンドオピニオン(往復検証)
      sanity-review/              対話コンテキスト込みの PR レビュー報告書(+ TEMPLATE.md)
      conversation-context-export/ 対話コンテキストの書き出し(.dev/contexts/ + PR コメント、+ TEMPLATE.md)
      conversation-context-import/ 対話コンテキストの読み込み
...
- **`TEMPLATE.md` は日本語のまま**: `agent-collaboration` の `sanity-review` /
  `conversation-context-export`。これは GitHub に貼る成果物の雛形(出力)であり、指示本体
  (`SKILL.md`)のみ英語化する。出力は利用者の作業言語に従う。
