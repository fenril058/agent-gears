...
- decision-interview: 計画・決定について一問ずつ推奨案付きでインタビューし、共通理解に達するまで実装に着手しない。
...
plugins/
  ...
  agent-collaboration/            plugin: エージェント協調(一部 shokai/agent-skills 由来・MIT)
    ...
    skills/
      ...
      decision-interview/         一問ずつ推奨案付きの意思決定インタビュー(mattpocock/skills 由来・MIT)
...
- **agent-collaboration**
  ...
  - `decision-interview` は [mattpocock/skills](https://github.com/mattpocock/skills) の `grilling` 由来。
    - ライセンスは **MIT**、`skills/decision-interview/LICENSE` 参照。
    - 上流は英語のみのため英語正本のまま取り込み、日本語ミラー `SKILL-ja.md` を追加した。
    - 名前は語勢を抑えて `decision-interview` に変更した(トリガーフレーズとして "grill me" は維持)。
